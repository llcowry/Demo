...
(function($) {
  $.fn.lxlTree = function() {
    return $.lxlui.run.call(this, "lxlTree", arguments);
  };
  $.fn.lxlTreeMgr = function() {
    return $.lxlui.run.call(this, "lxlTreeMgr", arguments);
  };
  $.lxlDefaults.Tree = {
    url: null,
    ...
    */
    ...
    onLoaded: function() {} //加载数据完事件
  };
  $.lxlui.controls.Tree = function(element, options) {
    $.lxlui.controls.Tree.base.constructor.call(this, element, options);
  };
  $.lxlui.controls.Tree.lxlExtend($.lxlui.core.UIComponent, {
    _init: function() {
      $.lxlui.controls.Tree.base._init.call(this);
      var g = this,
        p = this.options;
    ...
    _setTreeLine: function(value) {
      if (value) this.tree.removeClass("tree-noline");
      else this.tree.addClass("tree-noline");
    },
    _setParms: function() {
      var g = this,
        p = this.options;
    ...
    reloadNode: function(node, data, callback) { //刷新节点
      var g = this,
        p = this.options;
      g.clear(node);
    ...
    getParentTreeItem: function(treenode, level) { //获取父节点
      var g = this;
      treenode = g.getNodeDom(treenode);
      var treeitem = $(treenode);
      if (treeitem.parent().hasClass("tree")) {
        return null;
      }
      if (level === undefined) {
    ...
    getChecked: function() {
      ...
      var nodes = [];
      $(".checkbox-checked", g.tree).parent().parent("li").each(function() {
    ...
    getSelected: function() {
      var g = this,
        p = this.options;
      var node = {};
      node.target = $(".selected", g.tree).parent("li")[0];
      if (node.target) {
    ...
    upgrade: function(treeNode) { //升级为父节点级别
      var g = this,
        p = this.options;
      $(".note", treeNode).each(function() {
        $(this).removeClass("note").addClass("expandable-open");
      });
      $(".note-last", treeNode).each(function() {
        $(this).removeClass("note-last").addClass("expandable-open");
      });
    ...
    demotion: function(treeNode) { //降级为叶节点级别
      var g = this,
        p = this.options;
      if (!treeNode && treeNode[0].tagName.toLowerCase() != 'li') return;
      var islast = $(treeNode).hasClass("last");
      $(".expandable-open", treeNode).each(function() {
        $(this).removeClass("expandable-open")
          .addClass(islast ? "note-last" : "note");
      });
      $(".expandable-close", treeNode).each(function() {
        $(this).removeClass("expandable-close")
          .addClass(islast ? "note-last" : "note");
      });
    ...
    collapseAll: function() {
      var g = this,
        p = this.options;
      $(".expandable-open", g.tree).click();
    },
    expandAll: function() {
      var g = this,
        p = this.options;
      $(".expandable-close", g.tree).click();
    },
    ...
    clear: function(node) { //清空
      ...
      if (!node) {
        ...
        $(nodeDom).find("ul.children").remove();
        if (nodeData) nodeData.children = [];
      }
    },
    getNodeDom: function(nodeParm) { //parm [treeNode] dom节点(li)、节点数据 或者节点 dataindex
      var g = this,
        p = this.options;
    ...
    _updateStyle: function(ul) {
      ...
      itmes.each(function(i) {
        if (i === 0 && !$(this).hasClass("first"))
          $(this).addClass("first");
        if (i == treeitemlength - 1 && !$(this).hasClass("last"))
          $(this).addClass("last");
        if (i === 0 && i == treeitemlength - 1)
          $(this).addClass("onlychild");
        $("> div .note,> div .note-last", this)
          .removeClass("note note-last")
          .addClass(i == treeitemlength - 1 ? "note-last" : "note");
        g._setTreeItem(this, {
    ...
    update: function(domnode, newnodedata) {
      ...
      for (var attr in newnodedata) {
        ...
        if (attr == p.textFieldName) {
          $("> .tree-body > span", domnode).text(newnodedata[attr]);
        }
      }
    },
    //增加节点集合
    ...
    append: function(parentNode, newdata, nearNode, isAfter) {
      ...
      if (parentNode == null) { //增加到根节点
        ...
        $(".tree-body", g.tree).hover(function() {
          $(this).addClass("hover");
        }, function() {
      ...
      if (!hasChildren) {
        treeitem.append("<ul class='children'></ul>");
        g.upgrade(parentNode);
      }
      var isLast = [];
      for (var i = 1; i <= outlineLevel - 1; i++) {
        var currentParentTreeItem = $(g.getParentTreeItem(parentNode, i));
        isLast.push(currentParentTreeItem.hasClass("last"));
      }
      isLast.push(treeitem.hasClass("last"));
      ...
      if (nearNode != null) {
        ...
        if ($("> .children > li:last", treeitem).length > 0) {
          g._setTreeItem($("> .children > li:last", treeitem)[0], {
            isLast: false
          });
        }
        $(">.children", parentNode).append(gridhtmlarr.join(''));
      }
      g._upadteTreeWidth();
      $(">.children .tree-body", parentNode).hover(function() {
        $(this).addClass("hover");
      }, function() {
        $(this).removeClass("hover");
      });
    ...
    cancelSelect: function(nodeParm, isTriggerEvent) {
      ...
      if (p.checkbox)
        $(".checkbox", treeitembody).removeClass("checkbox-checked").addClass("checkbox-unchecked");
      else
        treeitembody.removeClass("selected");
    ...
    selectNode: function(selectNodeParm, isTriggerEvent) {
      ...
      if (typeof(selectNodeParm) == "function") {
        ...
        if (p.checkbox) {
          $(".checkbox", treeitembody).removeClass("checkbox-unchecked").addClass("checkbox-checked");
        } else {
          $("div.selected", g.tree).removeClass("selected");
          treeitembody.addClass("selected");
        }
        if (isTriggerEvent !== false) {
          g.trigger('select', [{
            data: treenodedata,
    ...
    _getDataNodeByTreeDataIndex: function(data, treedataindex) { //根据数据索引获取数据
      var g = this,
        p = this.options;
    ...
    _setTreeDataStatus: function(data, status) { //设置数据状态
      var g = this,
        p = this.options;
      $(data).each(function() {
    ...
    _addTreeDataIndexToData: function(data) { //设置data索引
      var g = this,
        p = this.options;
      $(data).each(function() {
    ...
    _appendData: function(treeNode, data) { //添加项到g.data
      var g = this,
        p = this.options;
      var treedataindex = parseInt($(treeNode).attr("treedataindex"));
      var treenodedata = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
    ...
    _setTreeItem: function(treeNode, options) {
      ...
      if (options.isLast !== undefined) {
        if (options.isLast === true) {
          treeItem.removeClass("last").addClass("last");
          $("> div .note", treeItem).removeClass("note").addClass("note-last");
          $(".children li", treeItem)
            .find(".tree-box:eq(" + (outlineLevel - 1) + ")")
            .removeClass("line");
        } else if (options.isLast === false) {
          treeItem.removeClass("last");
          $("> div .note-last", treeItem).removeClass("note-last").addClass("note");
          $(".children li", treeItem)
            .find(".tree-box:eq(" + (outlineLevel - 1) + ")")
            .removeClass("line")
            .addClass("line");
    ...
    _upadteTreeWidth: function() {
      ...
      if (p.nodeWidth) g.tree.width(treeWidth);
    },
    _getChildNodeClassName: function() {
      var g = this,
        p = this.options;
      return 'tree-icon-' + p.childIcon;
    },
    _getParentNodeClassName: function(isOpen) {
      var g = this,
        p = this.options;
      var nodeclassname = 'tree-icon-' + p.parentIcon;
      if (isOpen) nodeclassname += '-open';
      return nodeclassname;
    },
    _isExpand: function(o, level) { //判断节点是否展开状态
      var g = this,
        p = this.options;
    ...
    _getTreeHTMLByData: function(data, outlineLevel, isLast, isExpand) {
      ...
      else treehtmlarr.push("<ul class='children'>");
      for (var i = 0; i < data.length; i++) {
        var o = data[i];
        var isFirst = i === 0;
        ...
        treehtmlarr.push('"');
        treehtmlarr.push('>');
        treehtmlarr.push('<div class="tree-body');
        if (p.selectable && p.selectable(o) === false) {
          treehtmlarr.push(' unselectable');
        }
        treehtmlarr.push('">');
        for (var k = 0; k <= outlineLevel - 2; k++) {
          if (isLast[k]) treehtmlarr.push('<div class="tree-box"></div>');
          else treehtmlarr.push('<div class="tree-box line"></div>');
        }
        if (g.hasChildren(o)) {
          if (isExpandCurrent) treehtmlarr.push('<div class="tree-box expandable-open"></div>');
          else treehtmlarr.push('<div class="tree-box expandable-close"></div>');
          if (p.checkbox) {
            if (o.ischecked) {
              treehtmlarr.push('<div class="tree-box checkbox checkbox-checked"></div>');
            } else {
              treehtmlarr.push('<div class="tree-box checkbox checkbox-unchecked"></div>');
            }
          }
          if (p.parentIcon) {
            treehtmlarr.push('<div class="tree-box tree-icon ');
            treehtmlarr.push(g._getParentNodeClassName(isExpandCurrent ? true : false) + " ");
            if (p.iconFieldName && o[p.iconFieldName]) {
              treehtmlarr.push('tree-icon-none');
            } else if (p.iconClsFieldName && o[p.iconClsFieldName]) {
              treehtmlarr.push('tree-icon-' + o[p.iconClsFieldName] + " ");
            }
            treehtmlarr.push('">');
            if (p.iconFieldName && o[p.iconFieldName]) {
          ...
          if (isLastCurrent) treehtmlarr.push('<div class="tree-box note-last"></div>');
          else treehtmlarr.push('<div class="tree-box note"></div>');
          if (p.checkbox) {
            if (o.ischecked) {
              treehtmlarr.push('<div class="tree-box checkbox checkbox-checked"></div>');
            } else {
            ...
            if (delay === true) {
              g.toggleNodeCallbacks.push({
                ...
                callback: function(dom, o) {
                  ...
                  $(dom).append(content);
                  $(">.children .tree-body", dom).hover(function() {
                    $(this).addClass("hover");
                  }, function() {
                    $(this).removeClass("hover");
                  });
                ...
                g.toggleNodeCallbacks.push({
                  ...
                  callback: function(dom, o) {
                    g.loadData(dom, url, parms, {
                      showLoading: function() {
                        $("div.expandable-close:first", dom).addClass("tree-box-loading");
                      },
                      hideLoading: function() {
                        $("div.tree-box-loading:first", dom).removeClass("tree-box-loading");
                      }
                    });
                    g._removeToggleNodeCallback(o);
                  }
    ...
    _setTreeEven: function() {
      ...
      if (g.hasBind('contextmenu')) {
        g.tree.bind("contextmenu", function(e) {
          var obj = (e.target || e.srcElement);
          var treeitem = null;
          if (obj.tagName.toLowerCase() == "a" || obj.tagName.toLowerCase() == "span" || $(obj).hasClass("tree-box")) {
            treeitem = $(obj).parent().parent();
          } else if ($(obj).hasClass("tree-body")) {
            treeitem = $(obj).parent();
      ...
      g.tree.click(function(e) {
        ...
        var treedataindex = parseInt(treeitem.attr("treedataindex"));
        var treenodedata = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
        var treeitembtn = $("div.tree-body:first", treeitem).find("div.expandable-open:first,div.expandable-close:first");
        var clickOnTreeItemBtn = $(obj).hasClass("expandable-open") || $(obj).hasClass("expandable-close");
        if (!$(obj).hasClass("checkbox") && !clickOnTreeItemBtn) {
          if (!treeitem.hasClass("unselectable")) {
            if ($(">div:first", treeitem).hasClass("selected") && p.needCancel) {
              if (g.trigger('beforeCancelSelect', [{
                  data: treenodedata,
                  target: treeitem[0]
                }]) === false) return false;
              $(">div:first", treeitem).removeClass("selected");
              g.trigger('cancelSelect', [{
                data: treenodedata,
                target: treeitem[0]
              }]);
              ...
              $(".tree-body", g.tree).removeClass("selected");
              $(">div:first", treeitem).addClass("selected");
              g.trigger('select', [{
                data: treenodedata,
                target: treeitem[0]
              }]);
        ...
        if ($(obj).hasClass("checkbox")) {
          if (p.autoCheckboxEven) {
            //状态：未选中
            if ($(obj).hasClass("checkbox-unchecked")) {
              $(obj).removeClass("checkbox-unchecked").addClass("checkbox-checked");
              $(".children .checkbox", treeitem)
                .removeClass("checkbox-incomplete checkbox-unchecked")
            ...
            //状态：选中
            else if ($(obj).hasClass("checkbox-checked")) {
              $(obj).removeClass("checkbox-checked").addClass("checkbox-unchecked");
              $(".children .checkbox", treeitem)
                .removeClass("checkbox-incomplete checkbox-checked")
            ...
            //状态：未完全选中
            else if ($(obj).hasClass("checkbox-incomplete")) {
              $(obj).removeClass("checkbox-incomplete").addClass("checkbox-checked");
              $(".children .checkbox", treeitem)
                .removeClass("checkbox-incomplete checkbox-unchecked")
                .addClass("checkbox-checked");
              g.trigger('check', [{
            ...
            if ($(obj).hasClass("checkbox-unchecked")) {
              ...
              //是否单选
              if (p.single) {
                $(".checkbox", g.tree).not(obj).removeClass("checkbox-checked").addClass("checkbox-unchecked");
              }
              g.trigger('check', [{
                data: treenodedata,
                target: treeitem[0]
        ...
        //状态：已经张开
        else if (treeitembtn.hasClass("expandable-open") && (!p.btnClickToToggleOnly || clickOnTreeItemBtn)) {
          if (g.trigger('beforeCollapse', [{
              data: treenodedata,
              target: treeitem[0]
            }]) === false) return false;
          treeitembtn.removeClass("expandable-open").addClass("expandable-close");
          if (p.slide) {
            $("> .children", treeitem).slideToggle('fast');
          } else {
            $("> .children", treeitem).hide();
          }
        ...
        //状态：没有张开
        else if (treeitembtn.hasClass("expandable-close") && (!p.btnClickToToggleOnly || clickOnTreeItemBtn)) {
          ...
          treeitembtn.removeClass("expandable-close").addClass("expandable-open");
          ...
          if (p.slide) {
            $("> .children", treeitem).slideToggle('fast', callback);
          } else {
            $("> .children", treeitem).show();
            callback();
          }
      ...
      if ($.fn.lxlDrag && p.nodeDraggable) {
        g.nodeDroptip = $("<div class='drag-nodedroptip' style='display:none'></div>").appendTo('body');
        g.tree.lxlDrag({
          revert: true,
          animate: false,
          proxyX: 20,
          ...
          proxy: function(draggable, e) {
            ...
            if (src.node) {
              ...
              var proxy = $("<div class='drag-proxy' style='display:none'><div class='drop-icon drop-no'></div>" + content + "</div>").appendTo('body');
              return proxy;
            }
          },
          onRevert: function() {
          ...
          onDrag: function(current, e) {
            ...
            if (pageX < validRange.left || pageX > validRange.right || pageY > validRange.bottom || pageY < validRange.top) {
              g.nodeDropIn = -1;
              g.nodeDroptip.hide();
              this.proxy.find(".drop-icon:first").removeClass("drop-yes drop-add").addClass("drop-no");
              return;
            }
            for (var i = 0, l = g.nodes.length; i < l; i++) {
              ...
              if (pageX > range.left && pageX < range.right && pageY > range.top && pageY < range.bottom) {
                ...
                if (pageY > range.top + 7 && pageY < range.bottom - 7) {
                  this.proxy.find(".drop-icon:first").removeClass("drop-no drop-yes").addClass("drop-add");
                  g.nodeDroptip.hide();
                  g.nodeDropInParent = true;
                } else {
                  this.proxy.find(".drop-icon:first").removeClass("drop-no drop-add").addClass("drop-yes");
                  g.nodeDroptip.show();
    ...
    _setParentCheckboxStatus: function(treeitem) {
      var g = this,
        p = this.options;
      //当前同级别或低级别的节点是否都选中了
      var isCheckedComplete = $(".checkbox-unchecked", treeitem.parent()).length === 0;
      //当前同级别或低级别的节点是否都没有选中
      var isCheckedNull = $(".checkbox-checked", treeitem.parent()).length === 0;
      if (isCheckedNull) {
        treeitem.parent().prev().find("> .checkbox")
          .removeClass("checkbox-checked checkbox-incomplete")
          .addClass("checkbox-unchecked");
      } else {
        if (isCheckedComplete || !p.enabledCompleteCheckbox) {
          treeitem.parent().prev().find(".checkbox")
            .removeClass("checkbox-unchecked checkbox-incomplete")
            .addClass("checkbox-checked");
        } else {
          treeitem.parent().prev().find("> .checkbox")
            .removeClass("checkbox-unchecked checkbox-checked")
            .addClass("checkbox-incomplete");
        }
      }
      if (treeitem.parent().parent("li").length > 0) {
