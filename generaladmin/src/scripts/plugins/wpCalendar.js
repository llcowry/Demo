/**
 ***************************************************
 *MY97日历控件实现日期多选修改难度较大，改用梅花雨日历控件实现
 *MikeHu 2016年8月22日17:08:06
 ***************************************************
 */
...
var wpEvents = new function() {
  ...
  function __firefox(oWin) {
    ...
    oWin.constructor.prototype.__defineGetter__("event", function() {
      return self.__window_event(oWin);
    });
...
function wpCalendar() {
  ...
  try {
    this.IE6 = (parseFloat(window.navigator.appVersion.match(/MSIE (\d+\.\d+)/)[1]) >= 5.5)
  } catch (e) {
    this.IE6 = false
  }
  ...
  this.redraw = function() { //定义redraw方法，用于重绘整个日历
    ...
    with(this) {
      ...
      for (j = 0; j < 5; j++) {
        ...
        for (i = 0; i < 7; i++) {
          strFrame += '<td width=' + (cellwidth) + ' height=' + (cellheight) + ' id=wpDateCell' + n + '></td>';
          n++;
        }
        strFrame += '</tr>';
      }
      ...
      //strFrame += '        <td colspan=3 align=right class=calendarhead onclick=parent.wpCalendar.close() style="font-size:12px;cursor: pointer;padding-right:5px;" title="' + versioninfo + '&#13;' + theme.versioninfo + '">';
      //strFrame += '        <a href="javascript:void(0);">清空</a></td></tr>';
  ...
  this.writeCalendar = function() { //主要的写程序**********
    ...
    for (i = firstday; day1 < getDaysOfMonth(yy, mm) + 1; i++) { //本月的部分
      this.Days[i] = new wpDay(yy, mm, day1);
      day1++;
    }
    for (i = firstday + getDaysOfMonth(yy, mm); i < 39; i++) { //下个月的部分
      year = mm == 12 ? yy + 1 : yy;
      month = mm == 12 ? 1 : mm + 1;
      this.Days[i] = new wpDay(year, month, day2);
      day2++;
    }
    for (i = 0; i < 39; i++) {
      var da = this.DateCell[i];
      ...
      var dates = this.tgtObject.innerHTML.split("\n");
  ...
  this.dateCellClick = function(n) { //点击显示框选取日期，主输入函数*************
    ...
    var dates = this.tgtObject.value.split("\n");
    //var dates = this.tgtObject.innerHTML.split("\n");//【修改】实现textarea换行效果——MikeHu 2016-8-23 16:39:44
    if (dates.join("").length == 0 || dates[dates.length - 1] == "") {
      dates.pop();
    }
    ...
    if (this.tgtObject) {
      if (judge == -1) { //如果没有重复添加日期
        if (dates.length == 0) {
          ...
          //this.tgtObject.innerHTML += this.Days[n].date;//【修改】实现textarea换行效果——MikeHu 2016-8-23 16:39:44
        } else {
          this.tgtObject.value += "\n" + this.Days[n].date; //modified in Ver 2.51 设置input框中显示的日期拼接格式
          //this.tgtObject.innerHTML += "\n" + this.Days[n].date;//【修改】实现textarea换行效果——MikeHu 2016-8-23 16:39:44
        }
        var da = this.DateCell[n];
        this.DateCell[n].className = "selectedday";
        this.tgtObject.blur();
  ...
  function wpDay(year, month, day) {
    ...
    this.equals = function(eDate) {
      try {
        return this.year == eDate.getFullYear() && this.month == eDate.getMonth() + 1 && this.day == eDate.getDate();
      } catch (e) {
        return false
      }
    }
